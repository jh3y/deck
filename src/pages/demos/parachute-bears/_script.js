...
//   const delay = gsap.utils.mapRange(300, 650, 0.25, 0.75)(velocity);
...
//   });
//   gsap.set(OPTIONS, {
//     scaleY: Math.max(1, (this.__popoverHeight + this.y) / this.__popoverHeight),
//   });
// };
...
// const SPEED_SCALE = gsap.utils.mapRange(
//   SNAP_THRESHOLD * 0.5,
//   SNAP_THRESHOLD,
//   0.5,
//   0.25,
...
//   duration = gsap.utils.clamp(0.25, 0.5, SPEED_SCALE(this.y));

//   console.info({ y: this.y, duration });

//   // 1. Bounce back when under threshold (Spring)
...
//   ) {
//     // 2. Snap the popover :closed && initiate launch
//     // Snap closed please
//     closing = true;
//     count += 1;
//     // Duration * 1 / duration
//     // distance per seconds = (1 / duration) * distance
//     // The DPS is necessary so the popover springs closed at the right rate

//     DISTANCE_PER_SECOND = (1 / duration) * (this.__popoverHeight + this.y);
//     const DISTANCE = this.__popoverHeight * (this.y / this.__popoverHeight + 1);
...
const R2_SCREAM = new Audio("https://assets.codepen.io/605876/r2d2_scream.mp3");
          ...
          gsap.delayedCall(duration * 0.2, () => {
            ...
            if (velocity >= 630 && Math.random() > 0.5 && count > 3) {
              ...
              const JUMP = gsap
                ...
                .timeScale(2.2);
              gsap.delayedCall(delay + gsap.utils.random(-0.2, 0.2), () => {
                // JUMP.timeScale(gsap.utils.random(0.1, 0.25))
                gsap
                  .timeline({
                    onStart: () => {
                      if (Math.random() > 0.5) {
